...
def move_file(file_name, destination):
    ...
    if destination == ".." or destination == "<-":
        # Get full path to parent directory
        parent_path = "/".join(current_dir[:-1])
        
        # Check if parent exists
        if parent_path not in FS_data:
            return 2  # Parent directory doesn't exist
            
        # Check if file exists in parent
        if file_name in FS_data[parent_path]:
            return 4  # File already exists in parent
        ...
        FS_data[parent_path][file_name] = file_data
        FS_data.sync()
